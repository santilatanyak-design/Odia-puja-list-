 ...
 * Check if device or voter pin has already claimed free download
 */
export async function checkDeviceOrVoterClaimed(
  ...
  try {
    const claimedDevices = JSON.parse(localStorage.getItem('free_claims_devices') || '{}');
    if (claimedDevices[targetDeviceHash]) {
    ...
    if (voterIdPin) {
      ...
      if (cleanVoterPin && cleanVoterPin.length === 4) {
        const claimedVoters = JSON.parse(localStorage.getItem('free_claims_voters') || '{}');
        if (claimedVoters[cleanVoterPin]) {
...
/**
 * 3. Save Device Hash to Local Storage
...
export async function recordFreeDownloadClaim(
  ...
  try {
    const claimedDevices = JSON.parse(localStorage.getItem('free_claims_devices') || '{}');
    claimedDevices[deviceHash] = claimData;
    localStorage.setItem('free_claims_devices', JSON.stringify(claimedDevices));

    if (cleanVoterPin && cleanVoterPin !== '0000') {
      const claimedVoters = JSON.parse(localStorage.getItem('free_claims_voters') || '{}');
      claimedVoters[cleanVoterPin] = claimData;
      localStorage.setItem('free_claims_voters', JSON.stringify(claimedVoters));
    }
  } catch {}
}
...
export async function handleFreeDownloadWithDoubleLock(
  ...
  if (isDesktopDevice()) {
    try {
      let qrUrl = '';
      const cached = localStorage.getItem('qr_config_data');
      if (cached) {
        qrUrl = JSON.parse(cached)?.newCreationQrUrl || '';
      }
  ...
  try {
    // 2. Query Local Claim Check: Has this exact Device Hash already claimed a free download?
    const check = await checkDeviceOrVoterClaimed(deviceHash, voterIdPinInput);
    if (check.claimed) {
    ...
    // 3. First time (truly new mobile phone) -> Save Device Hash silently to storage
