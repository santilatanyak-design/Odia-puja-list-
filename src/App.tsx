...
export default function App() {
  ...
  const [selectedStoryId, setSelectedStoryId] = useState<string | null>(() => {
    try {
      if (typeof window !== 'undefined') {
        ...
        if (queryStoryId && queryStoryId.trim() && queryStoryId.trim() !== 'all') {
          return queryStoryId.trim();
        }

        // Priority 1: External social media links (?url=..., ?image=..., /story/..., etc.)
        const externalStory = extractExternalStoryParam();
        if (externalStory && externalStory !== 'all') {
  ...
  const [selectedStoryData, setSelectedStoryData] = useState<any | null>(() => {
    try {
      if (typeof window !== 'undefined') {
        const preloaded = (window as any).__PRELOADED_STATE__;
        if (preloaded && preloaded.story) return preloaded.story;
      }
    } catch {}
    return null;
  });
