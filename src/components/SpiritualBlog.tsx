...
export const SpiritualBlog: React.FC<SpiritualBlogProps> = ({
  ...
  // Comments State (Persistent across local and cloud storage)
  const [comments, setComments] = useState<any[]>([]);
  const [newCommentName, setNewCommentName] = useState<string>(() => {
    try {
      return localStorage.getItem('saved_viewer_comment_name') || '';
    } catch {
      return '';
    }
  });
  ...
  const [commentFeedback, setCommentFeedback] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const adTriggeredForStoryRef = useRef<Record<string, boolean>>({});
  const articleContainerRef = useRef<HTMLDivElement | null>(null);

  const [isResolvingTargetStory, setIsResolvingTargetStory] = useState<boolean>(false);

  const handleCloseAd = React.useCallback(() => {
    setIsAdOpen(false);
  }, []);

  const getCleanStoryId = React.useCallback((story: SpiritualStory | null | undefined): string => {
  ...
  // Fetch comments and poll periodically while story is open
  useEffect(() => {
    if (!selectedStory || !selectedStory.id) return;
    const cleanId = getCleanStoryId(selectedStory);
    if (!cleanId) return;

    let isMounted = true;

    const loadComments = () => {
      fetch(`/api/comments/${encodeURIComponent(cleanId)}?storyId=${encodeURIComponent(cleanId)}`)
        .then(res => res.json())
        .then(data => {
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    // Save viewer name for future visits
    try {
      localStorage.setItem('saved_viewer_comment_name', trimmedName);
    } catch {}
    ...
    try {
      const res = await fetch(`/api/comments/${encodeURIComponent(cleanId)}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          storyId: cleanId,
          originalStoryId: selectedStory.id,
          name: trimmedName,
          text: trimmedText
        })
      });
      ...
      if (data.success && Array.isArray(data.comments)) {
        ...
        setCommentFeedback({
          type: 'success',
          text: 'ଆପଣଙ୍କ ମତାମତ ସଫଳତାର ସହ ପ୍ରକାଶିତ ହେଲା! ଏହା ସମସ୍ତ ଦର୍ଶକଙ୍କୁ ଦୃଶ୍ୟମାନ ହେବ।'
        });
      } else {
      ...
      setCommentFeedback({
        type: 'error',
        text: 'ମତାମତ ପଠାଇବାରେ ତ୍ରୁଟି ହେଲା। ଦୟାକରି ପୁଣି ଚେଷ୍ଟା କରନ୍ତୁ।'
      });
    } finally {
  ...
  useEffect(() => {
    ...
    const resolveTargetStory = async () => {
      ...
      if (!targetId && typeof window !== 'undefined') {
        ...
        const queryStoryId = params.get('storyId') || params.get('story') || params.get('postId') || params.get('post') || params.get('id');
        ...
        targetId = queryStoryId || pathStoryId || externalParam || historyStoryId || preloadedId;
      }
      ...
      if (!cleanTargetId || cleanTargetId === 'all') return;

      // If already matching selectedStory, no need to refetch
      if (selectedStory && (selectedStory.id === targetId || selectedStory.id === cleanTargetId)) {
        return;
      }

      setIsResolvingTargetStory(true);

      try {
        // 1. Comprehensive multi-layer fetch (localStorage -> posts.json -> Firestore Doc -> Firestore REST -> S3 -> API)
        const storyFound = await fetchStoryByIdOrQuery(targetId);
        if (isCancelled) return;

        if (storyFound) {
          setSelectedStory(storyFound);
          return;
        }

        // 2. Check in current stories state if already loaded
        if (stories && stories.length > 0) {
          const matchedInStories = matchStoryFromExternalQuery(targetId, stories);
          if (matchedInStories) {
            setSelectedStory(matchedInStories);
            return;
          }
        }

        // 3. Check DEFAULT_STORIES fallback
      ...
      isFeatured: false
        ...
        if (!isCancelled) {
          setIsResolvingTargetStory(false);
        }
      }
    };
  ...
  // =========================================================================
  // LOADING VIEW (When resolving deep-linked post from social media)
  // =========================================================================
  if (isResolvingTargetStory && !selectedStory) {
    return (
      <div className="w-full max-w-4xl mx-auto px-4 py-24 flex flex-col items-center justify-center space-y-4 text-center">
        <div className="w-12 h-12 border-4 border-orange-200 border-t-orange-600 rounded-full animate-spin" />
        <p className="text-base sm:text-lg font-bold text-orange-950">
          ପୋଷ୍ଟ ଲୋଡ୍ ହେଉଛି... (Loading story...)
        </p>
        <p className="text-xs text-stone-500">
          ଭକ୍ତି ଆନନ୍ଦ ଓଡ଼ିଆ TV ଆଧ୍ୟାତ୍ମିକ କାହାଣୀ
        </p>
      </div>
    );
  }
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                  ...
                  <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-xl overflow-hidden bg-white border border-amber-200 shrink-0 shadow-xs flex items-center justify-center p-2">
                    <img
                      ...
                      alt={selectedStory.affiliateAd.productTitle || 'Bhakti Store Product'}
                  ...
                  <div className="min-w-0 flex-1 space-y-2 text-center sm:text-left">
                    ...
                    <div className="pt-2 flex flex-col sm:flex-row gap-2">
                      ...
                      {!selectedStory.affiliateAd.amazonLink && !selectedStory.affiliateAd.amazonLink && !selectedStory.affiliateAd.meeshoLink && selectedStory.affiliateAd.affiliateUrl && (
                        <button
                          type="button"
                          onClick={(e) => handleDeepLink(e, selectedStory.affiliateAd.affiliateUrl || 'https://www.Bhakti Store.in', 'generic')}
                          className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-600 via-amber-600 to-orange-700 hover:from-orange-500 hover:to-amber-500 text-white font-black text-xs sm:text-sm rounded-xl shadow-md hover:shadow-lg transition-all transform active:scale-95 w-full sm:w-auto cursor-pointer"
                        >
                          <ShoppingBag className="w-4 h-4 text-white" />
                          <span>Buy Now</span>
          ...
          <div className="mt-8 pt-6 border-t border-slate-200">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[11px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100 flex items-center gap-1">
                ...
                <span>ସମସ୍ତଙ୍କୁ ଦୃଶ୍ୟମାନ (Live)</span>
              </span>
            </div>
            ...
            <div className="space-y-3 mb-6">
              {comments.length > 0 ? (
                comments.map((comment: any) => {
                  const initial = (comment.name || 'ଦ').trim().charAt(0).toUpperCase();
                  let dateStr = '';
                  try {
                    const d = new Date(comment.createdAt);
                    if (!isNaN(d.getTime())) {
                      dateStr = d.toLocaleDateString('or-IN', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric',
                        hour: '2-digit',
                  ...
                  return (
                    <div key={comment.id || Math.random()} className="p-4 bg-white rounded-xl border border-slate-200/80 shadow-xs transition hover:border-orange-200">
                      <div className="flex justify-between items-center mb-2">
                        <div className="flex items-center gap-2.5">
                          <span className="w-7 h-7 rounded-full bg-gradient-to-br from-orange-400 to-amber-500 text-white font-black flex items-center justify-center text-xs shadow-xs">
                            {initial}
                          </span>
                          <div>
                      ...
                      <p className="text-slate-700 text-sm leading-relaxed pl-9 whitespace-pre-wrap">{comment.text}</p>
                    </div>
                  );
                })
              ) : (
                <div className="p-6 bg-amber-50/50 rounded-2xl border border-amber-200/60 text-center">
                  <p className="text-amber-900 text-sm font-semibold mb-1">ଏହି ପୋଷ୍ଟରେ ଏପର୍ଯ୍ୟନ୍ତ କୌଣସି ମତାମତ ନାହିଁ।</p>
                  <p className="text-amber-700/80 text-xs">ପ୍ରଥମେ ନିଜର ମତାମତ ଲେଖନ୍ତୁ, ସମସ୍ତ ଦର୍ଶକ ଏହା ଦେଖିପାରିବେ!</p>
                </div>
              )}
            </div>
            
            <form onSubmit={handleSubmitComment} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-3">
              <h4 className="text-sm font-bold text-slate-800 border-b border-slate-100 pb-2 mb-2 flex items-center justify-between">
                <span>ନିଜର ମତାମତ ଲେଖନ୍ତୁ (Leave a Comment)</span>
                <span className="text-[11px] text-slate-400 font-normal">ସେଭ୍ ହୋଇ ରହିବ</span>
              </h4>
              ...
              <div>
                <textarea
                  placeholder="ଆପଣଙ୍କ ମତାମତ ଏଠାରେ ଲେଖନ୍ତୁ (Your thoughts or prayers...)"
              ...
              <div className="pt-1 flex items-center justify-between">
                <button
                  ...
                  className="px-6 py-2.5 bg-orange-600 hover:bg-orange-700 disabled:opacity-50 text-white font-bold text-sm rounded-xl transition cursor-pointer shadow-xs active:scale-95 flex items-center gap-2"
                >
                  {isSubmittingComment ? (
                    <>
                    ...
                    <>
                      <span>💬</span>
                      <span>ମତାମତ ପଠାନ୍ତୁ (Post Comment)</span>
                    </>
                  )}
                </button>
                <span className="text-[11px] text-slate-400">
                  ସମସ୍ତ ଦର୍ଶକଙ୍କୁ ପ୍ରଦର୍ଶିତ ହେବ
                </span>
              </div>
            </form>
          </div>
